--	Module 7 DML指令的使用如何做資料的新增
--		7-1 : 使用 INSERT 新增資料
--		7-2 : 預設的日期表達格式
--		7-3 : 識別欄位(IDENTITY) 的使用 
...
-- 新增一筆資料
insert Member values ('使用者1', 'Mr.', '1-1-1999');   -- try insert into
select * from Member;
go
...
insert Member values ('使用者7', null, null);

select * from Member;
go

-- 新增一筆資料, Name 欄位值指定為 Unicode 編碼
insert Member (Name) values (N'使用者8');
