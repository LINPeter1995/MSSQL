--	Module 09. 使用者自訂函數(UDF) 的設計與實作    (User Defined Function)
--		09-1 : 如何建立純量函數(Scalar Functions)     
--		09-2 : 如何建立內嵌資料表值函數(Inline TVFs)     
--		09-3 : 如何建立多重語句資料表值函數(Multi-Statement TVFs)     
...
select LastName, FirstName, BirthDate,
			dbo.sf_myDate(BirthDate) as [BirthDate-10]					-- year -10
...
select dbo.discount_price(100, 3.12)  -- (price, %) 
...
--  修改已存在 function decimal (12, 4)
alter function dbo.discount_price(@price decimal(12, 2), @discount decimal(12, 2))				--	修改預存程序
...
exec sp_helptext 'dbo.discount_price'
