...
select coalesce (try_cast (PostalCode as int), 0), PostalCode  from Suppliers; 
select try_convert (int, PostalCode), PostalCode from Suppliers;							--  try to add "coalesce"
select try_parse (PostalCode as int using 'en-US'), PostalCode from Suppliers;	--  try to add "coalesce"
