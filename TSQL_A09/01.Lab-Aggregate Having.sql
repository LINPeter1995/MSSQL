--	Module 09 資料的分群以及彙總函數的使用
--		09-1 : 使用彙總函數(Aggregate Functions)
--		09-2 : 使用 GROUP BY 子句與 HAVING 子句
--		09-3 : SELECT敘述的進階查詢–使用 TOP 以及 OFFSET-FETCH 來篩選資料
...
select ProductID, sum(UnitPrice) as UPsum, sum(UnitsInStock) as UISsum
from Products
where SupplierID <6
group by ProductID
