 --	Module 15 子查詢(Subquery) 的運用
--		15-1 : 撰寫傳回單一純量值的子查詢
--		15-2 : 撰寫傳回多個值的子查詢
--		15-3 : 搭配 EXISTS 以及 NOT EXISTS 的子查詢
...
--where OrderID = (select OrderID from Orders where Freight= 51.30)
...
where exists (select * from [Order Details] where ProductID = p.ProductID and od.Quantity>12)
--where 1=1
