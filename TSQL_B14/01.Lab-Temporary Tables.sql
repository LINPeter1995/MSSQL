--  Module 14 暫存資料表(Temporary Table)與通用資料表運算式
--				(Common Table Expressions,CTE)的使用
--				14-1: 區域暫存資料表(Local Temporary Tables)
--				14-2: 全域暫存資料表(Global Temporary Tables)
--				14-3: 使用通用資料表運算式(Common Table Expressions)
...
--			@Table 與 #Table & ##Table的差異在於，前者不需使用DROP指令，當"此次指令執行完後" 會自動被刪除；
