--	Module 10. 透過 DML觸發程序(DML Triggers)來處理相關資料     
--		10-1:如何建立 AFTER INSERT Triggers     
--		10-2:如何建立 AFTER DELETE Triggers     
--		10-3:如何建立 AFTER UPDATE Triggers     
...
--								才引發DML 觸發程序。檢視 view 不能定義 AFTER 觸發程序。
...
insert Book values ('38', 'ado.net ', '2014-1-1', 33)
...
insert Book values ('19', 'SQL Server ', '2010-1-1', 120)		-- try PublisherID < 50
select * from Book
...
--啟用 DML 觸發
enab le trigger  tg_book_insert_select on Book;
