...
--	環境設定：
--		工具 → 選項 → 環境 → 字型和色彩 → 文字編輯器 → 字型,  大小
--		工具 → 選項 → 環境 → 字型和色彩 →   方格結果   → 字型,  大小
...
--			model		資料庫	用來當作 SQL Server 執行個體上建立之所有資料庫的 "範本"。 
...
--	伺服器名稱			●	資料庫名稱	●  結構描述名稱	● 資料表名稱
--	.\STUDENT-PC	●	LabDB	    	●	[dbo]Scheme	●	student
...
--	Steve 老師的 Q&A 社團 iCoding : https://www.facebook.com/groups/icoding
