...
--  再次檢視 : 資料庫 model → 檢視 → 系統檢視表 → sys.databases (右鍵)
...
--  當資料檔案寫滿，進行某些操作時，SQL Server會讓操作等待，直到檔案自動增長結束了，
--  原先的那個操作才能繼續進行。如果自增長用了很長時間，原先的操作會等不及就超時取消了
--  (一般預設的閾值是15秒)，不但這個操作會回滾，檔案自動增長也會被取消。
